...
//struct LessThanG {
//	bool operator()(const std::tuple<int, int, int>& e0, const std::tuple<int, int, int>& e1) const {
//		return (std::get<0>(e0) <= std::get<0>(e1)) || (std::get<0>(e0) == std::get<0>(e1) && std::get<1>(e0) < std::get<1>(e1));
//	}
//};
...
struct Mesh
{
	...
	std::vector<Hybrid_V> Vs;
	std::vector<Hybrid_E> Es;
	std::vector<Hybrid_F> Fs;
	std::vector<Hybrid> Hs;
};
...
struct Graph_Node
{
    ...
    std::unordered_set<int> nvs;
    std::unordered_set<int> nes;
...
{
    short dimension = 2;//3    
	std::vector<Graph_Node> Vs;
	std::vector<Graph_Edge> Es;
    std::vector<int> V_map;
    std::vector<std::unordered_set<int>> Reverse_V_map;
    std::vector<int> E_map;
    std::vector<std::unordered_set<int>> Reverse_E_map;
};
...
inline bool unordered_set_intersection_own(const std::unordered_set<int> &A, const std::unordered_set<int> &B, std::vector<int> &C)
{
	C.clear();
