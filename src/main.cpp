...
int main(int argc, char **argv) {
    ...
    int nprocs = -1;
		...
		bool Batch_Process = false, rosy= false;
		uint32_t  dim = 2;
		char batchIutput[300], batchOutput[300];
		Float scale = 1, tlen=1.0; uint32_t smooth_iter = 10;
    try {
	...
	if (Batch_Process) {
		batch_process(batchIutput, batchOutput, dim, tlen, scale, smooth_iter, rosy);
		return EXIT_SUCCESS;
	}
